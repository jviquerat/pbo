...
class Lattice:
    ...
    def add_buff(self, Cx, Cy, it):
        ...
        folder   = self.output_dir+'drag_lift/'
        if (not os.path.isdir(folder)): os.makedirs(folder)

        # Write to file
        filename = folder+str(self.ep)
    ...
    def output_fields(self, it, freq, *args, **kwargs):
        ...
        # Handle output folder
        folder   = self.output_dir+'fields/'+str(self.ep)+'/'
        if (not os.path.isdir(folder)): os.makedirs(folder)
        ...
        if (u_norm):
            ...
            filename = folder+'u_norm_'+str(self.output_it)+'.jpg'
            plt.axis('off')
    ...
    def generate_image(self):
        ...
        folder   = self.output_dir+'lattice/'
        if (not os.path.isdir(folder)): os.makedirs(folder)
        filename = folder+str(self.ep)+'.jpg'
